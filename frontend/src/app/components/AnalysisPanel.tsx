...
export function AnalysisPanel({
  ...
  const statusMeta = {
    ...
    failed: {
      label: 'Failed',
      className: 'bg-red-500/20 text-red-600 dark:text-red-300 border border-red-500/40',
      ...
      <div className="max-w-5xl mx-auto p-8 space-y-6">
        ...
        <div className="bg-muted/40 rounded-xl border border-border p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {metricEntries.map(([key, value]) => (
                  <div key={key} className="bg-muted/50 border border-border rounded-lg px-3 py-2 flex items-center justify-between gap-3">
                    <span className="text-xs text-muted-foreground uppercase tracking-wide">{key.replace(/_/g, ' ')}</span>
                    <span className="text-sm text-foreground font-medium">{formattedMetricValue(key, value)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {topImportanceEntries.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Feature Importance (Top 5)</h3>
              <div className="space-y-2">
                {topImportanceEntries.map(([feature, value]) => (
                  <div key={feature} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-foreground truncate">{feature}</span>
                      <span className="text-muted-foreground">{Number(value).toFixed(4)}</span>
                    </div>
          ...
          {importanceError && (
            <p className="text-sm text-amber-700 dark:text-amber-300/90 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">
              Feature importance note: {importanceError}
            </p>
          )}

          {trainingStatus === 'failed' && trainingError && (
            <p className="text-sm text-red-700 dark:text-red-300/90 bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2">
              Error: {trainingError}
            </p>
          )}
        </div>
        ...
        <div className="bg-muted/40 rounded-xl border border-border p-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold text-foreground">Run Counterfactual on New Input</h2>
        ...
        <div className="space-y-4">
          ...
          {analyses.length === 0 ? (
            <div className="bg-muted/40 rounded-xl border border-border p-8 text-center text-muted-foreground">
